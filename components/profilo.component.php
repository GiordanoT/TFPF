<?php
	...
	$resultEventi = getData("SELECT d.ora_inizio as ora_inizio, d.ora_fine as ora_fine, e.id as e_id, d.data as data_e, e.citta as citta_e, e.costo as costo_e, c.nome as catnome, e.posti as posti_e, e.descrizione as descrizione_e, e.nome as nome_e, e.immagine as immagine_e 
							FROM evento e JOIN data_evento d ON (d.id_evento = e.id) JOIN partecipazione p ON (p.id_data = d.id) JOIN categoria c ON (c.id = e.id_categoria) 
							WHERE p.id_utente = ".$_SESSION['id']." ORDER BY e.id");
