...
function VisualizzazioneEvento($evento){
    $risultatoEvento = getData("SELECT e.id as idevento, e.id_categoria as idcat, e.immagine as eveimmagine, c.immagine as catimmagine, e.nome as titolo, e.citta as citta, e.costo as prezzo,
		e.posti as posti, e.descrizione as descrizione, c.nome as nomecat, e.approvato as approvato FROM evento as e join categoria as c on (e.id_categoria = c.id ) where e.id ='{$evento}' ");
